...
``pdomain_book_contracts`` (moved here from ``pdomain_book_tools``) is the
foundation library every ``pdomain-*`` repo can depend on without an
imaging stack, so it is the cheapest single source of truth for
license-identifier validation. Without a shared allowlist each downstream
tool hand-rolls its own subset and they drift — one accepts
``Apache-2.0``, another typos it, and nothing catches the mismatch.
...
The data is vendored as ``pdomain_book_contracts/data/spdx_licenses.json``
(sourced from the SPDX ``license-list-data`` project) rather than
hand-curated, and rather than pulling in a third-party package -- a
runtime dependency here would ripple into every consumer's install
closure.
